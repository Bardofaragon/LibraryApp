...
%-------------------------------------- COMIENZA descripción del caso de uso.
	...
	\begin{UseCase}{CU06}{Registrar Cambio de Estado de un Libro}{
		Cuando un Usuario realiza  la devolución de un libro que previamente le fue prestado, el libro adquiere  una nueva disponibilidad, también si es necesario se registra un cambio en su estado físico (condición física del libro en el momento de la entrega), si las condiciones lo ameritan puede surgir una multa.
	}
		\UCitem{Versión}{0.1}
		\UCitem{Actor}{Administrador bibliotecario}
		\UCitem{Propósito}{Cambiar la disponibilidad de un libro después de un proceso de préstamo o entrega y registrar el estado físico que tiene el libro cuando se realiza la transición de estado.}
		...
		\UCitem{Salidas}{Se registra la fecha y hora del cambio de disponibilidad y una actualización en el estado físico si se presenta una variación. }
		...
		\UCitem{Postcondiciones}{El cambio en la disponibilidad de un libro o libros asociados a un préstamo  si existe un cambio en el estado físico  este  se asocia al cambio de disponibilidad en una fecha y hora específica.}
		\UCitem{Autor}{
			\begin{itemize}
	...
	\UCpaso[\UCsist] registra el cambio de disponibilidad del libro en la fecha y hora indicada.\Trayref{B}
